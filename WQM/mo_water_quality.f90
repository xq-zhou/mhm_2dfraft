...
CONTAINS
  ...
  subroutine wqm(&
    ...
    use mo_global_variables,     only: nLAIclass        ! number of LAI types (land use types)
    ...
    if (tt == 1)  then
      ...
      !here, rotation type map is modified from land use map due to technical simplification
       ...
       if (.not. read_restart) then
          ...
          soilMoisture, csoilMoist,cbaseflow, cunsatSTW, cpercol)    
       else
    ...
    do k = 1, nCells1
       ...
       !if ( (tt > 3653) .AND. (k==810) ) then
       !write(123,*) crunoff_sealed(k,1),cfastrunoff(k,1),cslowrunoff(k,1),cbaseflow(k,1),ctotal_runoff(k,1)
       !end if
    ...
    !if (k==1973) then   
    !write(24,*) crunoff_sealed(k,1), cfastrunoff(k,1), cslowrunoff(k,1), cbaseflow(k,1), ctotal_runoff(k,1) 
    !end if	
...
!sealedStorage+satStorage+
    !in-stream processes
    call conc_routing_process(tt,nNodes, nNodes-1 ,  timeStep, nPerm, nLink_from, nLink_to, nLink_length, &
  ...
  ! if (tt >6207) then
  ! write(23,*) aquatic_denitri(15), aquatic_assimil(15) 
  ! end if
  ...
  !>        \date Sep 2016
  subroutine conc_routing_process(tt,nNodes, nLinks, TS, nPerm, nLink_from, nLink_to, nLink_length,  &
       nNode_qOUT, nNode_qTR, nNode_concOUT, nNode_interload, nNode_concTIN, nLink_riverbox, nLink_criverbox, nLink_yravg_q, &
       rivertemp11, nNode_concMod, aquatic_denitri, aquatic_assimil,pardeniratew, parprodrate)
  
  use mo_wqm_global_variables,     only: L11_rivert_avg10, L11_rivert_avg20  !global state variables for moving average
  
  implicit none

  integer(i4),                     intent(in)    :: nNodes,tt
  ...
  integer(i4)             :: k,i
  real(dp)                :: width, depth, beneath_area   ![m] reach(link) width, depth and beneath area
  real(dp)                :: newbox
  integer(i4)             :: iNode, tNode
  real(dp), dimension(nNodes)  :: TinLoad  !total IN load_input
  real(dp), dimension(nNodes)  :: R_frac  !retention fraction
  ...
  do k=1, nLinks  
     ...
     nLink_yravg_q(i) = nLink_yravg_q(i) + (nNode_qTR(iNode) - nLink_yravg_q(i)) / (365.0_dp ) 
      ...
      !!!!!
     !To consider the cross section shape dependent on landuse type
     !if (landuse==arable) then
     
     !in the lower Bode the cross section shape close to 'rectangular'
     ! width = 9.74_dp * nLink_yravg_q(i)**(0.31_dp)
     ! depth = 0.27_dp * nLink_yravg_q(i)**(0.6_dp)
     
     ! else if (landuse==forest) then
     ! !assume in the headwater the stream cross section triangular shape 
     ! width = 9.74_dp * nLink_yravg_q(i)**(0.2_dp)
     ! depth = 0.27_dp * nLink_yravg_q(i)**(0.48_dp)
     
     ! !assume in the headwater the stream cross section shape triangular
     
     ! end if
     !!!!!
   
     ! empirial equations for channel width and depth
     !width = 9.74_dp * nLink_yravg_q(i)**(0.31_dp)  ! derive from observed cross  section
     
     ! if ((tt > 3653)) then
     ! write(123,*) i,width,nLink_yravg_q(i)
     ! end if
     !sensitivity analysis of width effect
     !!!!increase width by 10%
     !width = 1.5_dp*5.40_dp * nLink_yravg_q(i)**(0.50_dp)
     ...
     !write input load
     !if ((tt > 3653)  ) then
     !write(24,*) i, nLink_riverbox(i), nLink_criverbox(i,1), nNode_interload(iNode,1), &
      !nNode_concTIN(iNode,1)*temp_qTIN(iNode) * sec_TS,temp_qTIN(iNode), width, nLink_length(i)
      !end if
     ...
     call instream_nutrient_processes(tt,TS, iNode, rivertemp11(i), L11_rivert_avg10(i), L11_rivert_avg20(i), &
         beneath_area, depth, nLink_riverbox(i), nLink_criverbox(i,:),aquatic_denitri(i), aquatic_assimil(i), &
        pardeniratew(i), parprodrate(i))
     ...
     ! Retention amount= nLink_criverbox(i,:)*nLink_riverbox !!!before instream denitrification and primary production] 
        !                 - nLink_criverbox(i,:)*(nLink_riverbox + nNode_qTR(iNode) * sec_TS) !!!after instream denitrification
        !                                                                                     and primary production
      ! retention fraction = retentin amount/load_input !
      !if ((tt > 3653)  ) then
      ! write(23,*) i,nLink_riverbox(i),nNode_qTR(iNode)* sec_TS, nLink_criverbox(i,1),aquatic_denitri(i),aquatic_assimil(i)
      !end if
  ...
  !>        \param[in] "real(dp)                       :: beneath_area"  [m2]reach beneath area
  ...
  subroutine instream_nutrient_processes(tt,TS, iNode, rivertemp11, rt_avg10, rt_avg20, beneath_area, depth, &
        riverbox, criverbox, aqdenitri, aqassimil, pardeniratew, parprodrate)

		
  implicit none
  integer(i4),                    intent(in)     :: tt,TS, iNode
  real(dp),                       intent(in)     :: rivertemp11
  real(dp),                       intent(in)     :: rt_avg10
  real(dp),                       intent(in)     :: rt_avg20
  real(dp),                       intent(in)     :: beneath_area  
  ...
  ! if ((tt > 3653 ) ) then
    ! write(23,*) iNode,criverbox(1),aqdenitri, pardeniratew, f_temp, f_conc, beneath_area  
   ! end if  
  ...
  aqassimil =0.0_dp
  if (rivertemp11 >= 0.0_dp) then
     aqassimil = parprodrate * f_temp * f_tp * activedepth * depth * beneath_area/ DT !
  ...
  aqassimil = aqassimil
    ...
    ! if ((tt > 3653) ) then
    ! write(24,*) iNode,criverbox(1),aqassimil, parprodrate, f_temp, f_tp, depth, beneath_area  
    ! end if  
...
!  real(dp), dimension(:),         intent(in)     :: areaCell11   ! [km2] cell area at level L11  
 ...
 !     INTENT(INOUT)
  ...
  use mo_wqm_global_variables,   only: init_humusN, init_fastN, init_concIN, init_concON, hnhalf  
  ...
  integer(i4)       :: ir,jc, jc_prev    !rotation index/crop index in a specific rotation
  integer(i4)       :: remidr, remidr_prev    !reminder after mod function to identify cropid in a rotation
  ...
  do ir = 1, nCroptation
     if (frac_rotation(ir) > 0.0_dp ) then 
        ...
        ! growing period of winter crops in the next year, before Spring harvest 
        if (cropdata(jc_prev)%plantd > cropdata(jc_prev)%havestd .and. noday < cropdata(jc_prev)%havestd) then
           uptk_help = (cropdata(jc_prev)%up1 - cropdata(jc_prev)%up2) &
                       * exp(-cropdata(jc_prev)%up3*(noday - cropdata(jc_prev)%plantd + days_prev))           
           uptake_N = f_temp*cropdata(jc_prev)%up1 *cropdata(jc_prev)%up2*cropdata(jc_prev)%up3 * uptk_help &
                       /(1.0+uptk_help)/(1.0+ uptk_help)           
        end if
        !!normally plant in Spring and havest in Autumn
        ...
        !Winter crop, plant in Autumn and havest in Spring of next year, in other words, plant date is latter than havest date
        !in "cropdata.txt" we still use the day number of a year		
        else                                                     
           if (noday > cropdata(jc)%plantd ) then
           uptk_help = (cropdata(jc)%up1 - cropdata(jc)%up2) * exp(-cropdata(jc)%up3*(noday - cropdata(jc)%plantd-25))           
