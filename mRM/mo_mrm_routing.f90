...
CONTAINS
  ...
  subroutine mRM_routing( &
    ...
    call L11_runoff_acc(L1_total_runoff, L1_areaCell, L1_L11_Id, &
         ...
         map_flag, & ! Intent IN
         L11_qOut) ! Intent OUT
    ! check ID=785 meisdorf at 2km
    write(123,*) L1_total_runoff(785)
    ! add inflow
    ...
    write(126,*) gaugeNodeList(1),gaugeIndexList(1),L11_Qmod(gaugeNodeList(1))
  ...
  subroutine add_inflow(nInflowGauges, InflowIndexList, InflowHeadwater, &
    ...
    if (nInflowGauges .gt. 0) then
       do ii = 1, nInflowGauges
          if (.not.InflowHeadwater(ii)) then 
  ...
  subroutine L11_routing( &
    ...
    write(112,*) netNode_qTR(784,IT),netNode_qTIN(785,IT)
    !--------------------------------------------------------------------------
