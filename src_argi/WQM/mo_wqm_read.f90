...
!  call append(global_parameters, reshape(groundwater_residtime,(/1,nColPars/)))
  ...
  subroutine wqm_readinputdata()
    ...
    use mo_wqm_global_variables, only: dirInputWQM, nCroptation, &
         ...
         rotation,         & !rotation infos
         rotation_upper,        & !rotation infos in mountain arable (type variables)
         rotation_lower,        & !rotation infos in lower land arable (type variables)
         cropdata,         & !crop infos
         cropdata_upper,        & !crop infos in mountain arable (type variables)
         cropdata_lower,        & !crop infos in lower land arable (type variables)
         init_concIN,  & !
         init_concON,  & !
         init_humusN,  & !
         init_fastN,   & !
    ...
    !READ cropdata_upper
    !*****************
    fName = trim(adjustl(dirInputWQM(1))) // trim(adjustl("cropdata_upper.txt") )
    funit = 131
    open(funit, file = fName, action='read' )
    ...
    read(funit, *) dummy, num_crops
    read(funit, *) dummy
    allocate(cropdata_upper(num_crops) )
	
    do i =1, num_crops
       read(funit, *) cropdata_upper(i)%cropname, cropdata_upper(i)%cropid, cropdata_upper(i)%frtn1, cropdata_upper(i)%frtday1, &
       cropdata_upper(i)%frtdown1,cropdata_upper(i)%frtn2, cropdata_upper(i)%frtday2, cropdata_upper(i)%frtdown2, &
       cropdata_upper(i)%mann1, cropdata_upper(i)%manday1, cropdata_upper(i)%mandown1, cropdata_upper(i)%mann2,&
       cropdata_upper(i)%manday2, cropdata_upper(i)%mandown2, cropdata_upper(i)%manfIN,cropdata_upper(i)%frtperiod,  &
       cropdata_upper(i)%resn, cropdata_upper(i)%resday, cropdata_upper(i)%resdown, cropdata_upper(i)%resfast,   & 
       cropdata_upper(i)%resperiod,cropdata_upper(i)%up1, cropdata_upper(i)%up2, cropdata_upper(i)%up3, &
       cropdata_upper(i)%uppsoil, cropdata_upper(i)%plantd, cropdata_upper(i)%emergd, cropdata_upper(i)%havestd, &
       cropdata_upper(i)%ccrop, cropdata_upper(i)%ccplantd, cropdata_upper(i)%cchavestd

    !applied amount convert units: from kg/ha to kg/km2
    cropdata_upper(i)%frtn1 = cropdata_upper(i)%frtn1 * 100.0_dp 
    cropdata_upper(i)%frtn2 = cropdata_upper(i)%frtn2 * 100.0_dp
    cropdata_upper(i)%mann1 = cropdata_upper(i)%mann1 * 100.0_dp 
    cropdata_upper(i)%mann2 = cropdata_upper(i)%mann2 * 100.0_dp
    cropdata_upper(i)%resn  = cropdata_upper(i)%resn * 100.0_dp
    ...
    !READ cropdata_lower
    !*****************
    fName = trim(adjustl(dirInputWQM(1))) // trim(adjustl("cropdata_lower.txt") )
    funit = 132
    open(funit, file = fName, action='read' )
    ...
    read(funit, *) dummy, num_crops
    read(funit, *) dummy
    allocate(cropdata_lower(num_crops) )
	
    do i =1, num_crops
       read(funit, *) cropdata_lower(i)%cropname, cropdata_lower(i)%cropid, cropdata_lower(i)%frtn1, cropdata_lower(i)%frtday1, &
       cropdata_lower(i)%frtdown1, cropdata_lower(i)%frtn2, cropdata_lower(i)%frtday2, cropdata_lower(i)%frtdown2, &
       cropdata_lower(i)%mann1, cropdata_lower(i)%manday1, cropdata_lower(i)%mandown1, cropdata_lower(i)%mann2, &
       cropdata_lower(i)%manday2, cropdata_lower(i)%mandown2, cropdata_lower(i)%manfIN,cropdata_lower(i)%frtperiod, &
       cropdata_lower(i)%resn, cropdata_lower(i)%resday, cropdata_lower(i)%resdown, cropdata_lower(i)%resfast,&
       cropdata_lower(i)%resperiod,cropdata_lower(i)%up1, cropdata_lower(i)%up2, cropdata_lower(i)%up3, &
       cropdata_lower(i)%uppsoil, cropdata_lower(i)%plantd, cropdata_lower(i)%emergd, cropdata_lower(i)%havestd,&
       cropdata_lower(i)%ccrop, cropdata_lower(i)%ccplantd, cropdata_lower(i)%cchavestd

    !applied amount convert units: from kg/ha to kg/km2
    cropdata_lower(i)%frtn1 = cropdata_lower(i)%frtn1 * 100.0_dp 
    cropdata_lower(i)%frtn2 = cropdata_lower(i)%frtn2 * 100.0_dp
    cropdata_lower(i)%mann1 = cropdata_lower(i)%mann1 * 100.0_dp 
    cropdata_lower(i)%mann2 = cropdata_lower(i)%mann2 * 100.0_dp
    cropdata_lower(i)%resn  = cropdata_lower(i)%resn * 100.0_dp
    ...
    if (.not. allocated(rotation_lower) )   allocate(rotation_lower(nBasins) )
    if (.not. allocated(rotation_upper) )   allocate(rotation_upper(nBasins) )
    do iBasin =1, nBasins
    ...
    !*****************
    !READ rotation_lower INFORMATION 
    !*****************
    fName = trim(adjustl(dirInputWQM(iBasin) )) // trim(adjustl("rotation_lower_info.txt") )
    funit = 121
    open(funit, file = fName, action='read' )
    ...
    read(funit, *) dummy, nCroptation(iBasin)
    read(funit, *) dummy
    allocate(rotation_lower(iBasin)%id (nCroptation(iBasin)) )
    allocate(rotation_lower(iBasin)%ncrops (nCroptation(iBasin)) )
    allocate(rotation_lower(iBasin)%crop (nCroptation(iBasin), 10) )
    do i =1, nCroptation(iBasin)
       read(funit, *) rotation_lower(iBasin)%id(i), rotation_lower(iBasin)%ncrops(i), &
          (rotation_lower(iBasin)%crop(i, j),j=1, rotation_lower(iBasin)%ncrops(i)) 
    end do
    close(funit)
	!*****************
    !READ rotation_upper INFORMATION 
    !*****************
    fName = trim(adjustl(dirInputWQM(iBasin) )) // trim(adjustl("rotation_upper_info.txt") )
    funit = 122
    open(funit, file = fName, action='read' )
    ...
    read(funit, *) dummy, nCroptation(iBasin)
    read(funit, *) dummy
    allocate(rotation_upper(iBasin)%id (nCroptation(iBasin)) )
    allocate(rotation_upper(iBasin)%ncrops (nCroptation(iBasin)) )
    allocate(rotation_upper(iBasin)%crop (nCroptation(iBasin), 10) )
    do i =1, nCroptation(iBasin)
       read(funit, *) rotation_upper(iBasin)%id(i), rotation_upper(iBasin)%ncrops(i), &
          (rotation_upper(iBasin)%crop(i, j),j=1, rotation_upper(iBasin)%ncrops(i)) 
    end do
    close(funit)
    !*****************
