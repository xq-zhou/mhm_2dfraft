...
CONTAINS
  ...
  subroutine wqm(&
    ...
    use mo_global_variables,     only: nLAIclass, nSoilTypes,nGeoUnits, &  ! number of LAI types (land use types)/soil type/Geounits
                                       GeoUnitList
    use mo_wqm_global_variables, only: rotation_lower,year_start, day_preyr   ! simulation starting year		 
    ...
    if (tt == 1)  then
      ...
      !here, rotation type map is modified from land use map due to technical simplification
       do i=1, nCroptation
          frotation(:,i) = L0_fractionalCover_in_Lx( int(L0_cover_rotation), rotation_lower(iBasin)%id(i), mask0, &
                                               L0upBound_inL1,    &
                                               L0downBound_inL1,  &
                                               L0leftBound_inL1,  &
                                               L0rightBound_inL1, &
    ...
    !$OMP END PARALLEL
    !$ call message('Run WQM with OpenMP with ', trim(num2str(n_threads)), ' threads.')
    !*******************************
    !Soil phase
    !*******************************
    !$OMP parallel default(shared) &
    !$OMP private(k, cfastrunoff, dissIN, dissON)
...
!  real(dp), dimension(:),         intent(in)     :: areaCell11   ! [km2] cell area at level L11  
 ...
 !     INTENT(INOUT)
  ...
  use mo_wqm_global_variables,   only: &
       cropdata, cropdata_upper, cropdata_lower, &
       rotation, rotation_upper, rotation_lower, Geoform     !&
  ...
  potential_uptake = 0.0_dp
   ...
   if (frac_rotation(ir) > 0.0_dp ) then 
    ...
    ! abbreviation of crop rotation variables
       ...
       elseif ( ir ==nCroptation) then
       do i = 1, size(Geoform)
       if (Geoform(i) == 1) then
       ! abbreviation of crop rotation variables
        num_crp = rotation_lower(iBasin)%ncrops(ir)
        ! identify correct crop in specific year
        remidr = mod(noyear, num_crp)
        remidr_prev = mod(noyear-1, num_crp)
        remidr_next = mod(noyear+1, num_crp)  ! the year after, **modified 2019-07-17**
        ...
        if (remidr_prev == 0_i4)   remidr_prev = num_crp
        if (remidr_next == 0_i4)   remidr_next = num_crp
        jc = rotation_lower(iBasin)%crop(ir,remidr) 
        jc_prev = rotation_lower(iBasin)%crop(ir,remidr_prev)
        jc_next = rotation_lower(iBasin)%crop(ir,remidr_next)  !**modified 2019-07-17**
        !***get crop_id 'jc' from rotation_info.txt and then get corresponding crop management information from cropdata.txt***
        ! fertiliser application
        if ((noday >= cropdata_lower(jc)%frtday1 .and. noday < cropdata_lower(jc)%frtday1 + cropdata_lower(jc)%frtperiod) .or. &
            (noday < (cropdata_lower(jc_prev)%frtday1 + cropdata_lower(jc_prev)%frtperiod - days_prev))) then
        frtman_nadd(1,1) = frtman_nadd(1,1) + frac_rotation(ir) * cropdata_lower(jc)%frtn1 * (1.0 - cropdata_lower(jc)%frtdown1) &
                                            / (cropdata_lower(jc)%frtperiod * DT)
        frtman_nadd(2,1) = frtman_nadd(2,1) + frac_rotation(ir) * cropdata_lower(jc)%frtn1 * cropdata_lower(jc)%frtdown1  &
                                            / (cropdata_lower(jc)%frtperiod * DT)
        end if
        !in case fert applied twice a year
        if (cropdata_lower(jc)%frtday2 .ne. 0_i4) then
        if (noday >= cropdata_lower(jc)%frtday2 .and. noday < (cropdata_lower(jc)%frtday2 + cropdata_lower(jc)%frtperiod)) then
        frtman_nadd(1,1) = frtman_nadd(1,1) + frac_rotation(ir) * cropdata_lower(jc)%frtn2 * (1.0 - cropdata_lower(jc)%frtdown2) &
                                            / (cropdata_lower(jc)%frtperiod * DT)
        frtman_nadd(2,1) = frtman_nadd(2,1) + frac_rotation(ir) * cropdata_lower(jc)%frtn2 * cropdata_lower(jc)%frtdown2  &
                                            / (cropdata_lower(jc)%frtperiod * DT)
        end if
        end if
        !in case fert applied twice in previous year and the second application period across the calendar year
        if (cropdata_lower(jc_prev)%frtday2 .ne. 0_i4) then
        if (noday < (cropdata_lower(jc_prev)%frtday2 + cropdata_lower(jc_prev)%frtperiod - days_prev)) then
        frtman_nadd(1,1) = frtman_nadd(1,1) + frac_rotation(ir) * cropdata_lower(jc)%frtn2 * (1.0 - cropdata_lower(jc)%frtdown2) &
                                            / (cropdata_lower(jc)%frtperiod * DT)
        frtman_nadd(2,1) = frtman_nadd(2,1) + frac_rotation(ir) * cropdata_lower(jc)%frtn2 * cropdata_lower(jc)%frtdown2  &
                                            / (cropdata_lower(jc)%frtperiod * DT)
		...
		!manure application
        if ((noday >= cropdata_lower(jc)%manday1 .and. noday < cropdata_lower(jc)%manday1 + cropdata_lower(jc)%frtperiod) .or. &
            (noday < (cropdata_lower(jc_prev)%manday1 + cropdata_lower(jc_prev)%frtperiod - days_prev))) then
        frtman_nadd(1,1) = frtman_nadd(1,1) + frac_rotation(ir) * cropdata_lower(jc)%mann1 * (1.0 - cropdata_lower(jc)%mandown1) &
                                            * cropdata_lower(jc)%manfIN / (cropdata_lower(jc)%frtperiod * DT)
        frtman_nadd(2,1) = frtman_nadd(2,1) + frac_rotation(ir) * cropdata_lower(jc)%mann1 * cropdata_lower(jc)%mandown1  &
                                            * cropdata_lower(jc)%manfIN / (cropdata_lower(jc)%frtperiod * DT)
        frtman_nadd(1,2) = frtman_nadd(1,2) + frac_rotation(ir) * cropdata_lower(jc)%mann1 * (1.0 - cropdata_lower(jc)%mandown1) &
                                            * (1.0 - cropdata_lower(jc)%manfIN) / (cropdata_lower(jc)%frtperiod * DT)
        frtman_nadd(2,2) = frtman_nadd(2,2) + frac_rotation(ir) * cropdata_lower(jc)%mann1 * cropdata_lower(jc)%mandown1  &
                                            * (1.0 - cropdata_lower(jc)%manfIN) / (cropdata_lower(jc)%frtperiod * DT)
        end if
		!in case manure applied two times a year
        if (cropdata_lower(jc)%manday2 .ne. 0_i4) then
        if (noday >= cropdata_lower(jc)%manday2 .and. noday < (cropdata_lower(jc)%manday2 + cropdata_lower(jc)%frtperiod)) then
        frtman_nadd(1,1) = frtman_nadd(1,1) + frac_rotation(ir) * cropdata_lower(jc)%mann2 * (1.0 - cropdata_lower(jc)%mandown2) &
                                            * cropdata_lower(jc)%manfIN / (cropdata_lower(jc)%frtperiod * DT)
        frtman_nadd(2,1) = frtman_nadd(2,1) + frac_rotation(ir) * cropdata_lower(jc)%mann2 * cropdata_lower(jc)%mandown2  &
                                            * cropdata_lower(jc)%manfIN / (cropdata_lower(jc)%frtperiod * DT)
        frtman_nadd(1,2) = frtman_nadd(1,2) + frac_rotation(ir) * cropdata_lower(jc)%mann2 * (1.0 - cropdata_lower(jc)%mandown2) &
                                            * (1.0 - cropdata_lower(jc)%manfIN) / (cropdata_lower(jc)%frtperiod * DT)
        frtman_nadd(2,2) = frtman_nadd(2,2) + frac_rotation(ir) * cropdata_lower(jc)%mann2 * cropdata_lower(jc)%mandown2  &
                                            * (1.0 - cropdata_lower(jc)%manfIN) / (cropdata_lower(jc)%frtperiod * DT)
        end if
        end if
        !in case manure applied twice in previous year and the second application period across the calendar year	
        if (cropdata_lower(jc_prev)%manday2 .ne. 0_i4) then
        if (noday < (cropdata_lower(jc_prev)%manday2 + cropdata_lower(jc_prev)%frtperiod - days_prev))  then
        frtman_nadd(1,1) = frtman_nadd(1,1) + frac_rotation(ir) * cropdata_lower(jc)%mann2 * (1.0 - cropdata_lower(jc)%mandown2) &
                                            * cropdata_lower(jc)%manfIN / (cropdata_lower(jc)%frtperiod * DT)
        frtman_nadd(2,1) = frtman_nadd(2,1) + frac_rotation(ir) * cropdata_lower(jc)%mann2 * cropdata_lower(jc)%mandown2  &
                                            * cropdata_lower(jc)%manfIN / (cropdata_lower(jc)%frtperiod * DT)
        ...
        !residuals
        if (cropdata_lower(jc)%resday == 0_i4) cropdata_lower(jc)%resperiod = 365_i4
        if ((noday >= cropdata_lower(jc)%resday .and. noday < cropdata_lower(jc)%resday + cropdata_lower(jc)%resperiod) .or. &
            (noday < (cropdata_lower(jc_prev)%resday + cropdata_lower(jc_prev)%resperiod - days_prev))) then
        res_nadd(1,1) = res_nadd(1,1) + frac_rotation(ir) * cropdata_lower(jc)%resn * (1.0 - cropdata_lower(jc)%resdown) &
                                      * cropdata_lower(jc)%resfast / (cropdata_lower(jc)%resperiod * DT)
        res_nadd(1,2) = res_nadd(1,2) + frac_rotation(ir) * cropdata_lower(jc)%resn * (1.0 - cropdata_lower(jc)%resdown) &
                                      * (1.0 - cropdata_lower(jc)%resfast) / (cropdata_lower(jc)%resperiod * DT)
        res_nadd(2,1) = res_nadd(2,1) + frac_rotation(ir) * cropdata_lower(jc)%resn *  cropdata_lower(jc)%resdown &
                                      * cropdata_lower(jc)%resfast / (cropdata_lower(jc)%resperiod * DT)
        res_nadd(2,2) = res_nadd(2,2) + frac_rotation(ir) * cropdata_lower(jc)%resn *  cropdata_lower(jc)%resdown &
                                      * (1.0 - cropdata_lower(jc)%resfast) / (cropdata_lower(jc)%resperiod * DT)
        end if
        ...
        !!normally plant in Spring and harvest in Autumn
        if (cropdata_lower(jc)%plantd <= cropdata_lower(jc)%havestd) then    
           if (noday >= cropdata_lower(jc)%plantd .and. noday < cropdata_lower(jc)%havestd) then
           uptk_help = (cropdata_lower(jc)%up1 - cropdata_lower(jc)%up2) * &
           exp(-cropdata_lower(jc)%up3*(noday - cropdata_lower(jc)%plantd))
           uptake_N = cropdata_lower(jc)%up1 *cropdata_lower(jc)%up2*cropdata_lower(jc)%up3 * &
           uptk_help /(1.0+uptk_help)/(1.0+ uptk_help)
           end if
        end if
        !Winter crop, plant in Autumn of previous year and harvest in the current year, in other words, plant date is latter than harvest date
        ...
        ! in this case, the emerging date (emergd) should be specified in cropdata.txt
        if (cropdata_lower(jc)%plantd > cropdata_lower(jc)%havestd) then
           if (noday >= cropdata_lower(jc)%emergd .and. noday < cropdata_lower(jc)%havestd ) then
              uptk_help = (cropdata_lower(jc)%up1 - cropdata_lower(jc)%up2) * &
              exp(-cropdata_lower(jc)%up3*(noday - cropdata_lower(jc)%emergd))   
              uptake_N = cropdata_lower(jc)%up1 *cropdata_lower(jc)%up2 * &
              cropdata_lower(jc)%up3 * uptk_help/(1.0+uptk_help)/(1.0+ uptk_help)           
           elseif (noday < cropdata_lower(jc)%emergd) then
              uptk_help = (cropdata_lower(jc)%up1 - cropdata_lower(jc)%up2) * &
              exp(-cropdata_lower(jc)%up3*(noday + 365 - cropdata_lower(jc)%plantd))
              uptake_N = f_temp* cropdata_lower(jc)%up1 *cropdata_lower(jc)%up2 * &
              cropdata_lower(jc)%up3 * uptk_help/(1.0+uptk_help)/(1.0+ uptk_help)
           else
              uptake_N = 0.0_dp  
           end if
        ...
        ! if the next year cropping winter crops, means the next year's crop will be planted in current years' winter
        if (cropdata_lower(jc_next)%plantd > cropdata_lower(jc_next)%havestd) then  
           if (noday >= cropdata_lower(jc_next)%plantd ) then
           uptk_help = (cropdata_lower(jc_next)%up1 - cropdata_lower(jc_next)%up2) &
                       * exp(-cropdata_lower(jc_next)%up3*(noday - cropdata_lower(jc_next)%plantd)-25) !the "25" is directly introduced from HYPE
           uptake_N = f_temp*cropdata_lower(jc_next)%up1 *cropdata_lower(jc_next)%up2*cropdata_lower(jc_next)%up3* &
                         uptk_help /(1.0+uptk_help)/(1.0+ uptk_help)
           end if        
        end if

        potential_uptake(1) = potential_uptake(1) + frac_rotation(ir) * uptake_N * cropdata_lower(jc)%uppsoil 
        potential_uptake(2) = potential_uptake(2) + frac_rotation(ir) * uptake_N * (1.0 - cropdata_lower(jc)%uppsoil) 
    
       elseif( Geoform (i)== 2 ) then 
        ! abbreviation of crop rotation variables
        num_crp = rotation_upper(iBasin)%ncrops(ir)
        ! identify correct crop in specific year
        remidr = mod(noyear, num_crp)
        remidr_prev = mod(noyear-1, num_crp)
        remidr_next = mod(noyear+1, num_crp)  ! the year after, **modified 2019-07-17**
        ...
        if (remidr_prev == 0_i4)   remidr_prev = num_crp
        if (remidr_next == 0_i4)   remidr_next = num_crp
        jc = rotation_upper(iBasin)%crop(ir,remidr) 
        jc_prev = rotation_upper(iBasin)%crop(ir,remidr_prev)
        jc_next = rotation_upper(iBasin)%crop(ir,remidr_next)  !**modified 2019-07-17**
        !***get crop_id 'jc' from rotation_info.txt and then get corresponding crop management information from cropdata.txt***
        ! fertiliser application
        if ((noday >= cropdata_upper(jc)%frtday1 .and. noday < cropdata_upper(jc)%frtday1 + cropdata_upper(jc)%frtperiod) .or. &
            (noday < (cropdata_upper(jc_prev)%frtday1 + cropdata_upper(jc_prev)%frtperiod - days_prev))) then
        frtman_nadd(1,1) = frtman_nadd(1,1) + frac_rotation(ir) * cropdata_upper(jc)%frtn1 * (1.0 - cropdata_upper(jc)%frtdown1) &
                                            / (cropdata_upper(jc)%frtperiod * DT)
        frtman_nadd(2,1) = frtman_nadd(2,1) + frac_rotation(ir) * cropdata_upper(jc)%frtn1 * cropdata_upper(jc)%frtdown1  &
                                            / (cropdata_upper(jc)%frtperiod * DT)
        end if
        !in case fert applied twice a year
        if (cropdata_upper(jc)%frtday2 .ne. 0_i4) then
        if (noday >= cropdata_upper(jc)%frtday2 .and. noday < (cropdata_upper(jc)%frtday2 + cropdata_upper(jc)%frtperiod)) then
        frtman_nadd(1,1) = frtman_nadd(1,1) + frac_rotation(ir) * cropdata_upper(jc)%frtn2 * (1.0 - cropdata_upper(jc)%frtdown2) &
                                            / (cropdata_upper(jc)%frtperiod * DT)
        frtman_nadd(2,1) = frtman_nadd(2,1) + frac_rotation(ir) * cropdata_upper(jc)%frtn2 * cropdata_upper(jc)%frtdown2  &
                                            / (cropdata_upper(jc)%frtperiod * DT)
        end if
        end if
        !in case fert applied twice in previous year and the second application period across the calendar year
        if (cropdata_upper(jc_prev)%frtday2 .ne. 0_i4) then
        if (noday < (cropdata_upper(jc_prev)%frtday2 + cropdata_upper(jc_prev)%frtperiod - days_prev)) then
        frtman_nadd(1,1) = frtman_nadd(1,1) + frac_rotation(ir) * cropdata_upper(jc)%frtn2 * (1.0 - cropdata_upper(jc)%frtdown2) &
                                            / (cropdata_upper(jc)%frtperiod * DT)
        frtman_nadd(2,1) = frtman_nadd(2,1) + frac_rotation(ir) * cropdata_upper(jc)%frtn2 * cropdata_upper(jc)%frtdown2  &
                                            / (cropdata_upper(jc)%frtperiod * DT)
		...
		!manure application
        if ((noday >= cropdata_upper(jc)%manday1 .and. noday < cropdata_upper(jc)%manday1 + cropdata_upper(jc)%frtperiod) .or. &
            (noday < (cropdata_upper(jc_prev)%manday1 + cropdata_upper(jc_prev)%frtperiod - days_prev))) then
        frtman_nadd(1,1) = frtman_nadd(1,1) + frac_rotation(ir) * cropdata_upper(jc)%mann1 * (1.0 - cropdata_upper(jc)%mandown1) &
                                            * cropdata_upper(jc)%manfIN / (cropdata_upper(jc)%frtperiod * DT)
        frtman_nadd(2,1) = frtman_nadd(2,1) + frac_rotation(ir) * cropdata_upper(jc)%mann1 * cropdata_upper(jc)%mandown1  &
                                            * cropdata_upper(jc)%manfIN / (cropdata_upper(jc)%frtperiod * DT)
        frtman_nadd(1,2) = frtman_nadd(1,2) + frac_rotation(ir) * cropdata_upper(jc)%mann1 * (1.0 - cropdata_upper(jc)%mandown1) &
                                            * (1.0 - cropdata_upper(jc)%manfIN) / (cropdata_upper(jc)%frtperiod * DT)
        frtman_nadd(2,2) = frtman_nadd(2,2) + frac_rotation(ir) * cropdata_upper(jc)%mann1 * cropdata_upper(jc)%mandown1  &
                                            * (1.0 - cropdata_upper(jc)%manfIN) / (cropdata_upper(jc)%frtperiod * DT)
        end if
		!in case manure applied two times a year
        if (cropdata_upper(jc)%manday2 .ne. 0_i4) then
        if (noday >= cropdata_upper(jc)%manday2 .and. noday < (cropdata_upper(jc)%manday2 + cropdata_upper(jc)%frtperiod)) then
        frtman_nadd(1,1) = frtman_nadd(1,1) + frac_rotation(ir) * cropdata_upper(jc)%mann2 * (1.0 - cropdata_upper(jc)%mandown2) &
                                            * cropdata_upper(jc)%manfIN / (cropdata_upper(jc)%frtperiod * DT)
        frtman_nadd(2,1) = frtman_nadd(2,1) + frac_rotation(ir) * cropdata_upper(jc)%mann2 * cropdata_upper(jc)%mandown2  &
                                            * cropdata_upper(jc)%manfIN / (cropdata_upper(jc)%frtperiod * DT)
        frtman_nadd(1,2) = frtman_nadd(1,2) + frac_rotation(ir) * cropdata_upper(jc)%mann2 * (1.0 - cropdata_upper(jc)%mandown2) &
                                            * (1.0 - cropdata_upper(jc)%manfIN) / (cropdata_upper(jc)%frtperiod * DT)
        frtman_nadd(2,2) = frtman_nadd(2,2) + frac_rotation(ir) * cropdata_upper(jc)%mann2 * cropdata_upper(jc)%mandown2  &
                                            * (1.0 - cropdata_upper(jc)%manfIN) / (cropdata_upper(jc)%frtperiod * DT)
        end if
        end if
        !in case manure applied twice in previous year and the second application period across the calendar year	
        if (cropdata_upper(jc_prev)%manday2 .ne. 0_i4) then
        if (noday < (cropdata_upper(jc_prev)%manday2 + cropdata_upper(jc_prev)%frtperiod - days_prev))  then
        frtman_nadd(1,1) = frtman_nadd(1,1) + frac_rotation(ir) * cropdata_upper(jc)%mann2 * (1.0 - cropdata_upper(jc)%mandown2) &
                                            * cropdata_upper(jc)%manfIN / (cropdata_upper(jc)%frtperiod * DT)
        frtman_nadd(2,1) = frtman_nadd(2,1) + frac_rotation(ir) * cropdata_upper(jc)%mann2 * cropdata_upper(jc)%mandown2  &
                                            * cropdata_upper(jc)%manfIN / (cropdata_upper(jc)%frtperiod * DT)
        ...
        !residuals
        if (cropdata_upper(jc)%resday == 0_i4) cropdata_upper(jc)%resperiod = 365_i4
        if ((noday >= cropdata_upper(jc)%resday .and. noday < cropdata_upper(jc)%resday + cropdata_upper(jc)%resperiod) .or. &
            (noday < (cropdata_upper(jc_prev)%resday + cropdata_upper(jc_prev)%resperiod - days_prev))) then
        res_nadd(1,1) = res_nadd(1,1) + frac_rotation(ir) * cropdata_upper(jc)%resn * (1.0 - cropdata_upper(jc)%resdown) &
                                      * cropdata_upper(jc)%resfast / (cropdata_upper(jc)%resperiod * DT)
        res_nadd(1,2) = res_nadd(1,2) + frac_rotation(ir) * cropdata_upper(jc)%resn * (1.0 - cropdata_upper(jc)%resdown) &
                                      * (1.0 - cropdata_upper(jc)%resfast) / (cropdata_upper(jc)%resperiod * DT)
        res_nadd(2,1) = res_nadd(2,1) + frac_rotation(ir) * cropdata_upper(jc)%resn *  cropdata_upper(jc)%resdown &
                                      * cropdata_upper(jc)%resfast / (cropdata_upper(jc)%resperiod * DT)
        res_nadd(2,2) = res_nadd(2,2) + frac_rotation(ir) * cropdata_upper(jc)%resn *  cropdata_upper(jc)%resdown &
                                      * (1.0 - cropdata_upper(jc)%resfast) / (cropdata_upper(jc)%resperiod * DT)
        end if
        ...
        !!normally plant in Spring and harvest in Autumn
        if (cropdata_upper(jc)%plantd <= cropdata_upper(jc)%havestd) then    
           if (noday >= cropdata_upper(jc)%plantd .and. noday < cropdata_upper(jc)%havestd) then
           uptk_help = (cropdata_upper(jc)%up1 - cropdata_upper(jc)%up2) * &
           exp(-cropdata_upper(jc)%up3*(noday - cropdata_upper(jc)%plantd))
           uptake_N = cropdata_upper(jc)%up1 *cropdata_upper(jc)%up2 * &
           cropdata_upper(jc)%up3 * uptk_help /(1.0+uptk_help)/(1.0+ uptk_help)
           end if
        end if
        !Winter crop, plant in Autumn of previous year and harvest in the current year, in other words, plant date is latter than harvest date
        !in "cropdata_upper.txt" we still use the day number of a year
        ! in this case, the emerging date (emergd) should be specified in cropdata.txt
        if (cropdata_upper(jc)%plantd > cropdata_upper(jc)%havestd) then
           if (noday >= cropdata_upper(jc)%emergd .and. noday < cropdata_upper(jc)%havestd ) then
              uptk_help = (cropdata_upper(jc)%up1 - cropdata_upper(jc)%up2) * &
              exp(-cropdata_upper(jc)%up3*(noday - cropdata_upper(jc)%emergd))   
              uptake_N = cropdata_upper(jc)%up1 *cropdata_upper(jc)%up2* &
              cropdata_upper(jc)%up3 * uptk_help/(1.0+uptk_help)/(1.0+ uptk_help)           
           elseif (noday < cropdata_upper(jc)%emergd) then
              uptk_help = (cropdata_upper(jc)%up1 - cropdata_upper(jc)%up2) * &
              exp(-cropdata_upper(jc)%up3*(noday + 365 - cropdata_upper(jc)%plantd))
              uptake_N = f_temp* cropdata_upper(jc)%up1 *cropdata_upper(jc)%up2 * &
              cropdata_upper(jc)%up3 * uptk_help/(1.0+uptk_help)/(1.0+ uptk_help)
           else
              uptake_N = 0.0_dp  
           end if
        ...
        ! if the next year cropping winter crops, means the next year's crop will be planted in current years' winter
        if (cropdata_upper(jc_next)%plantd > cropdata_upper(jc_next)%havestd) then  
           if (noday >= cropdata_upper(jc_next)%plantd ) then
           uptk_help = (cropdata_upper(jc_next)%up1 - cropdata_upper(jc_next)%up2) &
                       * exp(-cropdata_upper(jc_next)%up3*(noday - cropdata_upper(jc_next)%plantd)-25) !the "25" is directly introduced from HYPE
           uptake_N = f_temp*cropdata_upper(jc_next)%up1 *cropdata_upper(jc_next)%up2*cropdata_upper(jc_next)%up3* &
                         uptk_help /(1.0+uptk_help)/(1.0+ uptk_help)
           end if        
        end if

        potential_uptake(1) = potential_uptake(1) + frac_rotation(ir) * uptake_N * cropdata_upper(jc)%uppsoil 
        potential_uptake(2) = potential_uptake(2) + frac_rotation(ir) * uptake_N * (1.0 - cropdata_upper(jc)%uppsoil) 
    
      
       end if !end of Geoform
       end do !end of Geoform loop
     end if  !end of ir
    end if !end of frotaion
